...
\textsc{ExactPoly} is not designed for high-performance computation.
The emphasis is on \textbf{clean design}, \textbf{comprehensibility}, and
\textbf{pedagogical transparency}: each class models exactly one mathematical
concept, the algebraic intent of every method is directly legible from its
implementation, and the library can serve as a readable reference for anyone
learning how symbolic polynomial arithmetic works from first principles.
For performance-critical applications, libraries such as
Rings~\cite{poslavsky2019} or JAS~\cite{kredel2008} are more appropriate.
...
ExactPoly is intentionally designed for clarity over speed.
This reflects a deliberate choice: the library's primary goals are clean
design, comprehensibility, and pedagogical transparency, not throughput.
Users who require high-performance polynomial arithmetic should consider
Rings~\cite{poslavsky2019} or JAS~\cite{kredel2008}.
Within this scope, three directions for future development are noted.
...
Its design prioritises clean structure, comprehensibility, and pedagogical
transparency over computational performance: each class models exactly one
mathematical concept, all types are immutable, and canonical form is maintained
